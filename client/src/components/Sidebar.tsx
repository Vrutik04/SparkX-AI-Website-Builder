...
import { BotIcon, EyeIcon, Loader2Icon, SendIcon, UserIcon, MessageSquareIcon, BlocksIcon } from 'lucide-react';
...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SidebarProps) => {
    ...
    useEffect(() => {
        if (messageRef.current && activeTab === 'chat') {
            messageRef.current.scrollIntoView({ behavior: 'smooth' })
        }
    }, [project.conversation.length, isGenerating, activeTab])

    return (
        <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all flex flex-col ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
            <div className='flex gap-1 p-2 mx-3 mt-3 bg-gray-950 rounded-lg border border-gray-800 uppercase tracking-wider shrink-0'>
                <button
                    onClick={() => setActiveTab('chat')}
                    className={`flex-1 flex gap-2 items-center justify-center py-2 text-xs font-semibold rounded-md transition-colors ${activeTab === 'chat' ? 'bg-gray-800 text-indigo-400' : 'text-gray-500 hover:text-gray-300'}`}
                >
                ...
                <button
                    ...
                    className={`flex-1 flex gap-2 items-center justify-center py-2 text-xs font-semibold rounded-md transition-colors ${activeTab === 'components' ? 'bg-gray-800 text-indigo-400' : 'text-gray-500 hover:text-gray-300'}`}
                >
            ...
            {activeTab === 'chat' ? (
                <div className='flex flex-col flex-1 overflow-hidden'>
                    {/* Messages container */}
                    <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4 pt-2'>
                ...
                <div className='flex-1 overflow-hidden'>
                    <ComponentGenerator projectId={project.id} onGenerateSuccess={fetchProject} />
                </div>
            )}
        </div>
    )

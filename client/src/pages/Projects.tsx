...
import { ArrowBigDownDashIcon, CodeIcon, EyeIcon, EyeOffIcon, FullscreenIcon, LaptopIcon, Loader2Icon, MessageSquareIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react'
...
const Projects = () => {
  ...
  // Download as Vite React project ZIP
  const exportReactProject = async () => {
    try {
      setIsExporting(true);
      const response = await api.get(`/api/export/${projectId}`, { responseType: 'blob' });
      const url = URL.createObjectURL(new Blob([response.data], { type: 'application/zip' }));
      const element = document.createElement('a');
      element.href = url;
      element.download = `${project?.name || 'sparkx-project'}.zip`;
      document.body.appendChild(element);
      element.click();
      document.body.removeChild(element);
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
              ...
              <button onClick={downloadCode} className='bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
                <ArrowBigDownDashIcon size={16} /> HTML
              </button>
              <button
                onClick={exportReactProject}
                disabled={isExporting || isGenerating}
                className='max-sm:hidden bg-linear-to-br from-violet-700 to-violet-600 hover:from-violet-600 hover:to-violet-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors disabled:opacity-50'
              >
                {isExporting ? <Loader2Icon size={16} className='animate-spin' /> : <CodeIcon size={16} />} React
              </button>
